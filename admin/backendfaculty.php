...
<?php
	...
	if(isset($_POST['readRecord'])){
		...
		$displayqry = "select * from faculty order by id DESC";
	...
	if(isset($_POST['readRecordsPending'])){
		...
		$displayqry = "select * from journal WHERE status = '0' order by id DESC";
		$result = mysqli_query($conn,$displayqry);
		if(mysqli_num_rows($result) > 0)
		{
	...
	if(isset($_POST['readRecordsAccepted'])){
		...
		$displayqry = "select * from journal WHERE status = '1' order by id DESC";
		$result = mysqli_query($conn,$displayqry);
		if(mysqli_num_rows($result) > 0)
		{
	...
	if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['mobile']) && isset($_POST['desig']) && isset($_POST['dept'])){
		$qry = "insert into faculty (name , email , mobile , designation , dept , username , password) values ('$name','$email','$mobile','$desig','$dept','$email','$pass')";
	...
	if(isset($_POST['val_email']))
	{
	    ...
	    $valquery = "SELECT * FROM faculty WHERE email = '$val_email'";
	...
	if(isset($_POST['deleteid'])){
		...
		$deleteqry = "delete from faculty where id = '$deleteid'";
	...
	if(isset($_POST['jor_id'])){
		...
		$updateqry = "UPDATE `journal` SET `status`= '1' WHERE `id` = '$jor_id'";
	...
	if(isset($_POST['jor_hide_id'])){
		...
		$updateqry = "UPDATE `journal` SET `status`= '0' WHERE `id` = '$jor_hide_id'";
		mysqli_query($conn,$updateqry);
	...
	if(isset($_POST['student_id'])){
		...
		$updateqry = "UPDATE `faculty` SET `password`= '$reset_pass' WHERE `id` = '$student_id'";
		mysqli_query($conn,$updateqry);
	...
	if(isset($_POST['updatename']) && isset($_POST['updateemail']) && isset($_POST['updatemobile']) && isset($_POST['updatedesig']) && isset($_POST['hidden_student_id']) )
	{

		$updateqry = "UPDATE `faculty` SET `name`= '$updatename' ,`email`= '$updateemail',`mobile`= '$updatemobile',`designation`= '$updatedesig',`username`= '$updateemail' WHERE `id` = '$hidden_student_id'";
	...
	if(isset($_POST['val_update_email']))
	{
	    ...
	    $updatevalquery = "SELECT * FROM faculty WHERE email = '$val_update_email' AND id <> '$hidden_student_id' ";
	...
	{
	    $viewid = $_POST['viewid'];
	    $viewquery = "SELECT * FROM faculty WHERE id = '$viewid'";
